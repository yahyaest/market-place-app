<script lang="ts">
	...
	export let productCommentReplies: Writable<Reply[][]> = writable([]);
	export let productReplies: Writable<
		{ commentId: number; commentReplies: Reply[] }[] | undefined
	> = writable([]);
	...
	const submitReply = async (commentId: number, commentIndex: number) => {
		...
		// Update UI
		const commentReplies = [submittedReply, ...$productCommentReplies[commentIndex]];
