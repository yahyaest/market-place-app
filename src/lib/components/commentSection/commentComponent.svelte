<script lang="ts">
	...
	export let commentUpVotes: Writable<number> = writable(0);
	export let commentDownVotes: Writable<number> = writable(0);
	export let isCommentUpVote: Writable<boolean | Vote> = writable(false);
	export let isCommentDownVote: Writable<boolean | Vote> = writable(false);
	...
	const submitVote = async (voteType: 'UP' | 'DOWN') => {
		...
		if (voteType === 'UP') {
			if ($isCommentUpVote) {
				await deleteVote(commentBaseUrl, token, $isCommentUpVote.id);
				isCommentUpVote.update((bool) => false);
				commentUpVotes.update((i) => i - 1);
			} else {
				if ($isCommentDownVote) {
					await deleteVote(commentBaseUrl, token, $isCommentDownVote.id);
					isCommentDownVote.update((bool) => false);
					commentDownVotes.update((i) => i - 1);
				}
				const vote = (await addVote(commentBaseUrl, token, voteType, commentData.id, null)) as Vote;
				isCommentUpVote.update((val) => vote);
				commentUpVotes.update((i) => i + 1);
			}
		}
		...
		if (voteType === 'DOWN') {
			if ($isCommentDownVote) {
				...
				const vote = (await addVote(commentBaseUrl, token, voteType, commentData.id, null)) as Vote;
				isCommentDownVote.update((val) => vote);
				commentDownVotes.update((i) => i + 1);
			}
		}
	};
			...
			<p class="inline-flex items-center mr-3 text-sm font-semibold">
				<img
					...
					src={commentData.userImage
						? `${data.gatewayBaseUrl}/${commentData.userImage}`
	...
	<div class="flex items-center mt-4 space-x-4">
		...
		<button
			...
			class={`flex items-center text-sm  hover:underline  font-medium ${
				$isCommentUpVote ? 'text-cyan-500 ' : 'text-gray-500'
			} `}
			on:click={() => {
				submitVote('UP');
			}}
		...
		<button
			type="button"
			class={`flex items-center text-sm  hover:underline  font-medium ${
				$isCommentDownVote ? 'text-cyan-500 ' : 'text-gray-500'
			} `}
			on:click={() => {
				submitVote('DOWN');
			}}
