<script lang="ts">
	...
	import { addComment, addReply, getThreadByName } from '../../../service/comment';
	...
	let productCommentReplies: Writable<Reply[][]> = writable([]);
	let productReplies: Writable<{ commentId: number; commentReplies: Reply[] }[] | undefined> =
		writable([]);
	...
	const getProductReplies = async () => {
		const replies = data.replies;
		productReplies.set(replies);
	};

	const submitReply = async (commentId: number, commentIndex: number) => {
		const commentBaseUrl = data.commentBaseUrl as string;
		const gatewayBaseUrl = data.gatewayBaseUrl as string;
		const token = Cookies.get('token') as string;
		const userImage = (await getCurrentUserAvatar(gatewayBaseUrl)) as string;
		...
		const commentReplies = [
			submittedReply,
			...$productCommentReplies[commentIndex]
		];

		const currentReplies = [...$productCommentReplies];
		currentReplies[commentIndex] = commentReplies;
		productCommentReplies.update((arr) => arr = [...currentReplies]);

		productReplies.update((arr) => {
			const index = arr?.findIndex((e) => e.commentId === commentId);
			if (index !== -1 && typeof index === 'number' && arr) {
				arr[index].commentReplies = commentReplies;
			}
		});
	...
	let repliesInputs = new Array($productComments.length).fill(false);
	const toggleRepliesInputs = (index: number) => {
		if (repliesInputs[index]) {
			repliesInputs = new Array($productComments.length).fill(false);
		} else {
			repliesInputs = new Array($productComments.length).slice(index, 0).fill(false);
			repliesInputs[index] = !repliesInputs[index];
		}
	};

	let openCommentDropdowns = new Array($productComments.length).fill(false);
	const toggleCommentDropdown = (index: number) => {
		if (openCommentDropdowns[index]) {
			openCommentDropdowns = new Array($productComments.length).fill(false);
		} else {
			openCommentDropdowns = new Array($productComments.length).slice(index, 0).fill(false);
			openCommentDropdowns[index] = !openCommentDropdowns[index];
		}
	};

	let openReplyDropdowns = new Array(100).fill(false);
	const toggleReplyDropdown = (index: number, commentId: number) => {
		const replies = $productReplies?.find((e) => e.commentId === commentId)?.commentReplies;
		if (replies) {
			openReplyDropdowns = new Array(replies.length).fill(false);
			if (openReplyDropdowns[index]) {
				openReplyDropdowns = new Array(replies.length).fill(false);
			} else {
				openReplyDropdowns = new Array(replies.length).slice(index, 0).fill(false);
				openReplyDropdowns[index] = !openReplyDropdowns[index];
			}
		}
	};

	const checkCommentReplies = (commentId: number) => {
		try {
			$productReplies?.find((e) => e.commentId === commentId)?.commentReplies as Reply[];
			return true;
		} catch (error) {
			return false;
		}
			...
			<section class=" py-8 lg:py-16 antialiased">
				<div class="max-w-2xl mx-auto px-4">
					...
					<form class="mb-6">
						...
						<button type="submit" class="btn btn-primary" on:click={submitComment}>
							Post comment
						</button>
					</form>
					{#each $productComments as comment, index (comment)}
						<article class="p-6 mb-3 text-base bg-white rounded-lg dark:bg-gray-900 rounded-lg">
							<footer class="flex justify-between items-center mb-2">
								...
								{#if comment.userEmail === JSON.parse(Cookies.get('user')).email}
									<button
							...
							<div class="flex items-center mt-4 space-x-4">
								<button
									...
									on:click={() => toggleRepliesInputs(index)}
								>
									<svg
							...
							<div class:hidden={!repliesInputs[index]}>
								<input
									type="text"
									id="text"
								...
								<button
									...
									on:click={() => submitReply(comment.id, index)}
								>
									Post Reply
								</button>
							</div>
						</article>
						{#if checkCommentReplies(comment.id) && repliesInputs[index]}
									...
									<footer class="flex justify-between items-center mb-2">
										<div class="flex items-center">
											...
											>
												<img
													class="mr-2 w-6 h-6 rounded-full"
													src={reply.userImage ? reply.userImage : '/images/default_avatar.webp'}
													alt="Jese Leos"
												/>{reply.username}
											</p>
											<p class="text-sm text-gray-600 dark:text-gray-400">
										...
										{#if reply.userEmail === JSON.parse(Cookies.get('user')).email}
											<button
